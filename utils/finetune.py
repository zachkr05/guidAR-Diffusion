...
def finetune_online(
    ...
    attract_m, repulse_m = build_edit_masks(user_t, orig_t, region_mask, device=device)

    user_p    = user_t / (user_t.sum() + 1e-8)            # attract target (sums to 1)
    repulse_p = repulse_m / (repulse_m.sum() + 1e-8)      # repulse target (0 if no divergence)
